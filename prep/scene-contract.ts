// prep/scene-contract.ts — reference artifact. Fable copies this into src/types/scene.ts
// in Phase 0. It is the shared primitive every scene reuses; changing it means updating
// all scene configs in one commit (the S&N house rule).

import type { Dataset, SyntheticEvent } from "../src/types/data"; // authored in Phase 1
// ON COPY to src/types/scene.ts, rewrite this import to "./data" (co-located there).
// TaskClass / RuleConcept imported from vocab at build; shown here for clarity.
...
export interface SceneConfig {
	...
	eventKinds: readonly SyntheticEvent["kind"][]; // streams this scene reads (see DATA-MODEL §5)
...
// Registry shape (src/scenes/index.ts): SCENES: Record<string, SceneConfig>; SCENE_ORDER: readonly slug[].
// The global session clock lives in a context provider ABOVE <RouterProvider> so it survives route
// switches; scenes read eventsUpTo(t)/eventsOfKind(kind,t)/missionEvents(id,t) from it.
